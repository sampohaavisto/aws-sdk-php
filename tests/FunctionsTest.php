...
class FunctionsTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $iter = Aws3\recursive_dir_iterator(__DIR__);
    ...
    {
        $iter = Aws3\dir_iterator(__DIR__);
        $this->assertInstanceOf('Iterator', $iter);
        $files = iterator_to_array($iter);
    ...
    {
        Aws3\load_compiled_json('/path/to/not/here.json');
    }
    ...
    public function testUsesPhpCompilationOfJsonIfPossible()
    {
        ...
        $this->assertSame($soughtData, Aws3\load_compiled_json($jsonPath));
    }

    public function filterTest()
    {
        $data = [0, 1, 2, 3, 4];
        $result = \Aws3\filter($data, function ($v) { return $v % 2; });
    ...
    {
        $data = [0, 1, 2, 3, 4];
        $result = \Aws3\map($data, function ($v) { return $v + 1; });
